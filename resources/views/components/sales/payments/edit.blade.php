...
@section('primary_action')
    <a href="{{ route('sales.payments.show', $row['number']) }}"
...
@section('content')
    <div class="max-w-3xl space-y-4">
        ...
        <form method="POST" action="{{ route('sales.payments.update', $row['number']) }}" class="bg-white border border-slate-200 rounded-2xl p-5 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="payment_date" value="{{ old('payment_date', $row['date']) }}"
                ...
                <div>
                    ...
                    <select name="method"
                        ...
                        @foreach($methods as $method)
                            <option value="{{ $method }}" @selected(old('method', $row['method']) === $method)>
                                {{ $method }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="reference" value="{{ old('reference', $row['reference']) }}"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>

                <div>
                    <label class="text-xs text-slate-500">Amount *</label>
                    <input type="number" min="0.01" step="1" name="amount" value="{{ old('amount', $row['amount']) }}"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="4"
                              class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">{{ old('notes', $row['notes']) }}</textarea>
                </div>
            </div>
