        ...
        <form method="POST" action="{{ route('sales.payments.store') }}" class="bg-white border border-slate-200 rounded-2xl p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="payment_number" value="{{ old('payment_number') }}" placeholder="e.g. PAY-00045"
                ...
                <div>
                    ...
                    <input type="text" name="reference" value="{{ old('reference') }}" placeholder="e.g. NBS-TRX-39401"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>

                <div>
                    <label class="text-xs text-slate-500">Amount *</label>
                    <input type="number" min="0.01" step="1" name="amount" value="{{ old('amount') }}"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="4"
                              class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200"
                              placeholder="Optional internal note">{{ old('notes') }}</textarea>
                </div>
            </div>
