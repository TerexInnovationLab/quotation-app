...
@section('primary_action')
    <a href="{{ route('sales.receipts.show', $row['number']) }}"
...
@section('content')
    <div class="max-w-3xl space-y-4">
        ...
        <form method="POST" action="{{ route('sales.receipts.update', $row['number']) }}" class="bg-white border border-slate-200 rounded-2xl p-5 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="receipt_date" value="{{ old('receipt_date', $receipt['receipt_date']) }}"
                ...
                <div>
                    ...
                    <input type="text" name="customer_name" value="{{ old('customer_name', $receipt['customer_name']) }}"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>
                ...
                <div>
                    ...
                    <select name="method"
                            class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                        @foreach(['Bank Transfer', 'Cash', 'Card', 'Mobile Money'] as $method)
                            <option value="{{ $method }}" @selected(old('method', $receipt['method']) === $method)>{{ $method }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="reference" value="{{ old('reference', $receipt['reference']) }}"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>

                <div>
                    <label class="text-xs text-slate-500">Amount *</label>
                    <input type="number" min="0.01" step="1" name="amount" value="{{ old('amount', $receipt['amount']) }}"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        @foreach(['Draft', 'Issued', 'Sent'] as $status)
                            <option value="{{ $status }}" @selected(old('status', $receipt['status']) === $status)>{{ $status }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="4"
                              class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">{{ old('notes', $receipt['notes']) }}</textarea>
                </div>
            </div>
