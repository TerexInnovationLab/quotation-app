...
@section('primary_action')
    <a href="{{ route('sales.quotations.show', $row['number']) }}"
...
@section('content')
    <div class="max-w-4xl space-y-4">
        ...
        <form method="POST" action="{{ route('sales.quotations.update', $row['number']) }}" class="bg-white border border-slate-200 rounded-2xl p-5 space-y-4">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="text-xs text-slate-500">Quotation #</label>
                ...
                <div>
                    ...
                    <input type="date" name="quotation_date" value="{{ old('quotation_date', $row['date']) }}"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>

                <div>
                    <label class="text-xs text-slate-500">Expiry Date</label>
                    <input type="date" name="expiry_date" value="{{ old('expiry_date', $row['expiry'] ?? '') }}"
                           class="mt-1 w-full rounded-xl border-slate-200 focus:border-slate-400 focus:ring-slate-200">
                </div>
